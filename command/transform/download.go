...
func fetch(reference string, repositories map[string]map[string]string, progress func(transferred, total int64)) (string, error) {
	origin := origin(reference)

	if repository, ok := repositories[origin.Host]; ok {
